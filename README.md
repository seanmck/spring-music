...
The Database driver for SQL Server will need to be downloaded from https://www.microsoft.com/en-us/download/details.aspx?id=11774.  Once the jar is downloaded, it needs to be installed in your local maven repository in order to be packaged with the jar.  This can be done through the command 

```
mvn install:install-file -Dfile=sqljdbc6.jar -DgroupId=com.microsoft.sqlserver -DartifactId=sqljdbc6 -Dversion=6.0 -Dpackaging=jar
```

This will enable the SqlServerServiceInfo class to be correctly plugged into the Cloud configuration classes to generate a SQLServer data source if you have a bound service of a SQL Server type as generated by the Azure Services tile.
